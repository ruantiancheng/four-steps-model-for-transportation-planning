...
def read_info(file_name, zonal_name):
    ...
    return zonal_point_info, point_info
        ...
        for j in range(zonal_number):
            if i == j:
                ...
                while activate:
                    ...
                    print( ai2 / ai, bj2 / bj)
                    if 0.6 < ai2 / ai < 1.4 and 0.6 < bj2 / bj < 1.4:
                        activate = False
                    else:
                        ai = ai2
                        bj = bj2
...
def main():
    ...
    zonal_point_info, point_info = read_info(file_name, zonal_name)  # 读取小区文件中，每个小区周边的节点信息
    ...
    while i < 3:
        ...
        model_split_matrix.to_csv("方式划分矩阵.csv", encoding="gbk")
        ...
        road_cost, dir_link_df, link_df = assignment(file_name, zonal_name, road_cost, distribution_matrix,
                                                     dir_link_df,
                                                     origin_dir_link, zonal_point_info, point_info)
    ...
    link_df.to_csv("路段流量表.csv", encoding="gbk")
    distribution_matrix.to_csv("分布矩阵.csv", encoding="gbk")
