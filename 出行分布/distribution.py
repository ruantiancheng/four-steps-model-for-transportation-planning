    ...
    for col in range(0, len(zonal_point_info)):  # 检索每个交通小区的OD，这一步先不做，只做小区1 到小区2的路径
        for loc in range(1, len(zonal_point_info) + 1):
            ...
            if col + 1 != loc:#起终点小区不一致
                ...
                print(zonal_cost)
                zonal_min_dir = min(zonal_cost, key=zonal_cost.get)
                print(dir_link_df)
                print(dir_link_df[zonal_min_dir[0]][zonal_min_dir[1]])

                # while number < distribution_matrix.iloc[col, loc]:  # 以小区2 到小区1 作为示范进行交通量
                #     for j in range(len(dir_link_df[col + 1][loc]) - 1):
                #         link_df.loc[((link_df.A == dir_link_df[col + 1][loc][j]) & (
                #                 link_df.B == dir_link_df[col + 1][loc][j + 1])) | ((
                #                                                                            link_df.B ==
                #                                                                            dir_link_df[col + 1][loc][
                #                                                                                j]) & (link_df.A ==
                #                                                                                       dir_link_df[
                #                                                                                           col + 1][loc][
                #                                                                                           j + 1])), 'v'] += 10
                #         work_link = link_df.loc[((link_df.A == dir_link_df[col + 1][loc][j]) & (
                #                 link_df.B == dir_link_df[col + 1][loc][j + 1])) | ((
                #                                                                            link_df.B ==
                #                                                                            dir_link_df[col + 1][loc][
                #                                                                                j]) & (link_df.A ==
                ...
                #                                                                                           j + 1]))]
                #
                #         origin_work_link = origin_link_df.loc[((origin_link_df.A == dir_link_df[col + 1][loc][j]) & (
                #                 origin_link_df.B == dir_link_df[col + 1][loc][j + 1])) | ((
                #                                                                                   origin_link_df.B ==
                #                                                                                   dir_link_df[col + 1][
                #                                                                                       loc][
                #                                                                                       j]) & (
                ...
                #
                #         link_df.loc[  # 计算新阻抗
                #             (link_df.A == dir_link_df[col + 1][loc][j]) & (
                #                     link_df.B == dir_link_df[col + 1][loc][j + 1]) | ((
                #                                                                               link_df.B ==
                #                                                                               dir_link_df[col + 1][loc][
                #                                                                                   j]) & (link_df.A ==
                ...
                #
                #         link_time[dir_link_df[col + 1][loc][j]][dir_link_df[col + 1][loc][j + 1]] = new_lw_time
                #         link_time[dir_link_df[col + 1][loc][j + 1]][dir_link_df[col + 1][loc][j]] = new_lw_time
                ...
                #                                                                                           col + 1][loc][
                #                                                                                           j + 1])), 'v'] += \
                #             distribution_matrix.iloc[col, loc] + 10 - number
                #         work_link = link_df.loc[((link_df.A == dir_link_df[col + 1][loc][j]) & (
                #                 link_df.B == dir_link_df[col + 1][loc][j + 1])) | ((
                #                                                                            link_df.B ==
                #                                                                            dir_link_df[col + 1][loc][
                ...
                #         print(work_link)
                #         new_lw_time = bpr_caculation(
                #             float(work_link.loc[0]['lw_time']), (float(work_link.loc[0]['v']) / work_link.loc[0]['c']))
                #
                #         link_df.loc[  # 计算新阻抗
                #             (link_df.A == dir_link_df[col + 1][loc][j]) & (
                #                     link_df.B == dir_link_df[col + 1][loc][j + 1]), 'lw_time'] = new_lw_time
                #
                #         link_time[dir_link_df[col + 1][loc][j]][dir_link_df[col + 1][loc][j + 1]] = new_lw_time
                #         link_time[dir_link_df[col + 1][loc][j + 1]][dir_link_df[col + 1][loc][j]] = new_lw_time
                #         newdir_link, new_dir_link_df = zdy_dijkstra(link_time)
...
def main():
    ...
    assignment_matrix = assignment(file_name, zonal_name, road_cost, distribution_matrix, dir_link_df,
