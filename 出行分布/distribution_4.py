...
'''
    ...
    问题：重力模型里gamma值没有修正，只做了单约束没做双约束导致PA不平衡
        问题出在路径不均衡上，考虑怎么在路径垂直同步后完成中间节点读取（index？）
* ━━━━━━神兽出没━━━━━━
* 　　　┏┓　　　┏┓
* 　　┏┛┻━━━┛┻┓
* 　　┃　　　　　　　┃
...
def gravity_model(traffic_cost, PAmatrix, gamma=9999, exp_gamma=[1, 5, 10, 15, 20,35, 50],
                  exp_travelcost=[3600, 3000, 2600, 1300, 750, 250, 120]):
    '''
        重力模型
    ...
    # gamma = gamma_cal()
    print(traffic_cost)
    traffic_cost = traffic_cost[traffic_cost != 0].apply(lambda x: x ** -gamma_cal(x))
