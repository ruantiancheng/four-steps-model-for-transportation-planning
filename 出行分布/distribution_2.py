...
def assignment(file_name, zonal_name, traffic_cost, distribution_matrix, dir_link_df, link_time,zonal_point_info):
    zonal_point_info_pd = pd.Series(zonal_point_info)
    ...
    for start in range(len(traffic_cost)):  # 检索每个交通小区的OD，这一步先不做，只做小区1 到小区2的路径
        for end in range(1,len(traffic_cost)+1):
            for item_a in zonal_point_info_pd[start]:
                for item_b in zonal_point_info_pd[end-1]:
                    number = 0
                    while number < distribution_matrix.iloc[start, end]:  # 以小区2 到小区1 作为示范进行交通量
                        for j in range(len(dir_link_df[start + 1][end]) - 1):
                            link_df.loc[((link_df.A == dir_link_df[start + 1][end][j]) & (
                                    link_df.B == dir_link_df[start + 1][end][j + 1])) | ((
                                                                                               link_df.B ==
                                                                                               dir_link_df[start + 1][end][
                                                                                                   j]) & (link_df.A ==
                                                                                                          dir_link_df[
                                                                                                              start + 1][end][
                                                                                                              j + 1])), 'v'] += 10
                            work_link = link_df.loc[((link_df.A == dir_link_df[start + 1][end][j]) & (
                                    link_df.B == dir_link_df[start + 1][end][j + 1])) | ((
                                                                                               link_df.B ==
                                                                                               dir_link_df[start + 1][end][
                                                                                                   j]) & (link_df.A ==
                                                                                                          dir_link_df[
                                                                                                              ...
                                                                                                              j + 1]))]
                            print(work_link)
                            origin_work_link = origin_link_df.loc[((origin_link_df.A == dir_link_df[start + 1][end][j]) & (
                                    origin_link_df.B == dir_link_df[start + 1][end][j + 1])) | ((
                                                                                                      origin_link_df.B ==
                                                                                                      dir_link_df[start + 1][end][
                                                                                                          j]) & (
                                                                                                      origin_link_df.A ==
                            ...
                            new_lw_time = bpr_caculation(
                                float(origin_work_link.loc[0]['lw_time']), (float(work_link.loc[0]['v']) / work_link.loc[0]['c']))

                            link_df.loc[  # 计算新阻抗
                                (link_df.A == dir_link_df[start + 1][end][j]) & (
                                        link_df.B == dir_link_df[start + 1][end][j + 1]) | ((
                                                                                                  link_df.B ==
                                                                                                  dir_link_df[start + 1][end][
                                                                                                      j]) & (link_df.A ==
                                                                                                             dir_link_df[
                                                                                                                 start + 1][end][
                                                                                                                 j + 1])), 'lw_time'] = new_lw_time

                            link_time[dir_link_df[start + 1][end][j]][dir_link_df[start + 1][end][j + 1]] = new_lw_time
                            link_time[dir_link_df[start + 1][end][j + 1]][dir_link_df[start + 1][end][j]] = new_lw_time
                            newdir_link, new_dir_link_df = zdy_dijkstra(link_time)
                        number += 10
            print(link_df)
            # else:
            #     for j in range(len(dir_link_df[start + 1][end]) - 1):
            #         link_df.end[((link_df.A == dir_link_df[start + 1][end][j]) & (
            #                 link_df.B == dir_link_df[start + 1][end][j + 1])) | ((
            #                                                                            link_df.B ==
            #                                                                            dir_link_df[start + 1][end][
            #                                                                                j]) & (link_df.A ==
            ...
            #                                                                                           start + 1][end][
            #                                                                                           j + 1])), 'v'] += \
            #         distribution_matrix.iloc[start, end] + 10 - number
            #         work_link = link_df.end[((link_df.A == dir_link_df[start + 1][end][j]) & (
            #                 link_df.B == dir_link_df[start + 1][end][j + 1])) | ((
            #                                                                            link_df.B ==
            #                                                                            dir_link_df[start + 1][end][
            #                                                                                j]) & (link_df.A ==
            ...
            #         print(work_link)
            #         new_lw_time = bpr_caculation(
            #             float(work_link.end[0]['lw_time']), (float(work_link.end[0]['v']) / work_link.end[0]['c']))
            #
            #         link_df.end[  # 计算新阻抗
            #             (link_df.A == dir_link_df[start + 1][end][j]) & (
            #                     link_df.B == dir_link_df[start + 1][end][j + 1]), 'lw_time'] = new_lw_time
            #
            #         link_time[dir_link_df[start + 1][end][j]][dir_link_df[start + 1][end][j + 1]] = new_lw_time
            #         link_time[dir_link_df[start + 1][end][j + 1]][dir_link_df[start + 1][end][j]] = new_lw_time
            #         newdir_link, new_dir_link_df = zdy_dijkstra(link_time)
    print(link_df)
    print(new_dir_link_df)
...
def main():
    ...
    link_time = read_shp(file_name, 9)
    origin_dir_link = copy.deepcopy(link_time)

    road_cost, dir_link_df = zdy_dijkstra(link_time)
    zonal_point_info = read_info(file_name, zonal_name)
    traffic_cost = zonal_inter_link(road_cost, zonal_point_info)
    ...
    assignment_matrix = assignment(file_name, zonal_name, traffic_cost, distribution_matrix, dir_link_df,
                                   origin_dir_link,zonal_point_info)

    # print(distribution_matrix.plot)
